...
class SlackToChatMigrator:
    ...
    def _initialize_api_services(self):
        ...
        # Initialize thread-local storage for api services
        self._thread_local.chat = None
        self._thread_local.drive = None
    ...
    def _initialize_dependent_services(self):
        ...
        # self.current_channel is now a property using thread_local storage
        ...
        # Track current message timestamp for enhanced error context using thread-local storage
        self._thread_local.current_message_ts = None
    ...
    def chat(self):
        """
        Thread-safe access to Google Chat API service.
        Returns thread-local service if available, otherwise main thread service.
        """
        # Check if we have a thread-local service
        if hasattr(self._thread_local, "chat") and self._thread_local.chat:
            return self._thread_local.chat
            
        # If we are in a worker thread but have no service, verify if we should create one
        # For simplicity, we'll check if we are NOT in the main thread
        if threading.current_thread() is not threading.main_thread():
            if self._api_services_initialized:
                # Initialize thread-local service
                creds_path_str = str(self.creds_path)
                self._thread_local.chat = get_gcp_service(
                    creds_path_str, self.workspace_admin, "chat", "v1", 
                    retry_config=self.config,
                    use_cache=False,  # Force new instance for thread safety
                )
                return self._thread_local.chat
        
        # Fallback to main thread service (or return None if not initialized)
        return getattr(self, "_main_chat", None)

    @chat.setter
    def chat(self, value):
        """Setter for compatibility with existing code during initialization."""
        self._main_chat = value

    @property
    def drive(self):
        """
        Thread-safe access to Google Drive API service.
        Returns thread-local service if available, otherwise main thread service.
        """
        # Check if we have a thread-local service
        if hasattr(self._thread_local, "drive") and self._thread_local.drive:
        ...
        if threading.current_thread() is not threading.main_thread():
            if self._api_services_initialized:
                # Initialize thread-local service
                creds_path_str = str(self.creds_path)
                self._thread_local.drive = get_gcp_service(
                    creds_path_str,
                    self.workspace_admin,
                    "drive",
                    "v3",
        ...
        return getattr(self, "_main_drive", None)

    @drive.setter
    def drive(self, value):
        """Setter for compatibility with existing code during initialization."""
        self._main_drive = value
    
    @property
    def current_channel(self):
        """Thread-safe access to current channel."""
        return getattr(self._thread_local, "current_channel", None)

    @current_channel.setter
    def current_channel(self, value):
        """Thread-safe setter for current channel."""
        self._thread_local.current_channel = value
        
    @property
    def current_space(self):
        """Thread-safe access to current space."""
        return getattr(self._thread_local, "current_space", None)

    @current_space.setter
    def current_space(self, value):
        """Thread-safe setter for current space."""
        self._thread_local.current_space = value

    @property
    def current_message_ts(self):
        """Thread-safe access to current message timestamp."""
        return getattr(self._thread_local, "current_message_ts", None)

    @current_message_ts.setter
    def current_message_ts(self, value):
        """Thread-safe setter for current message timestamp."""
        self._thread_local.current_message_ts = value
    ...
    def migrate(self):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                # Submit all channels
                future_to_channel = {
                    executor.submit(self._process_single_channel, ch): ch 
                    for ch in all_channel_dirs
                }
                
                # Check for keyboard interrupt to stop gracefully
                try:
                    # Process results as they complete
                    for future in concurrent.futures.as_completed(future_to_channel):
                        ch = future_to_channel[future]
                        channel_name = unicodedata.normalize("NFC", ch.name)
                        try:
                            future.result()
                        except Exception as exc:
                            log_with_context(
                                logging.ERROR,
                                f"Channel {channel_name} generated an exception: {exc}",
                                exc_info=True,
                                channel=channel_name
                            )
                        
                        # Check if execution stopped (e.g. abort condition in one of the threads)
                        if hasattr(self, "stop_execution") and self.stop_execution.is_set():
                            log_with_context(
                                logging.WARNING,
                                "Execution stop signal received. Cancelling pending tasks...",
                            )
                            kwargs = {"wait": False}
                    ...
                    log_with_context(
                        logging.WARNING,
                        "Migration interrupted by user (Ctrl+C). Stopping workers...",
                    )
                    ...
                    # Re-raise to let outer handler generate report
                    raise
    ...
    def _process_single_channel(self, ch: Path) -> None:
        """
        Process a single channel in a thread-safe manner.
        
        Args:
            ch: Path to the channel directory
        """
        # Normalize channel name to NFC to ensure it matches keys in channels.json
        # macOS file system uses NFD, but JSON usually contains NFC
        channel_name = unicodedata.normalize("NFC", ch.name)

        # Set thread-local current channel
        ...
        log_with_context(
            logging.INFO,
            f"{mode_prefix if self.dry_run or self.update_mode else ''}Processing channel: {channel_name}",
            channel=channel_name,
        )

        with self.lock:
            self.migration_summary["channels_processed"].append(channel_name)

        # Check if we should abort (if another thread raised a critical error)
        if hasattr(self, "stop_execution") and self.stop_execution.is_set():
            return

        # Check if channel should be processed
        if not should_process_channel(channel_name, self.config):
            log_with_context(
                logging.WARNING,
                f"Skipping channel {channel_name} based on configuration",
                channel=channel_name,
            )
            return
        ...
        if (
            ...
            and channel_name in self.channel_conflicts
        ):
            log_with_context(
                logging.ERROR,
                f"Skipping channel {channel_name} due to unresolved duplicate space conflict",
                channel=channel_name,
            )
            return
        ...
        channel_handler = setup_channel_logger(
            self.output_dir, channel_name, self.verbose, is_debug_api_enabled()
        )
        
        # We need to track handlers to close them later
        # Since we're in a thread, we should probably close it at end of this method
        # But for now, let's add to main list with lock
        with self.lock:
            self.channel_handlers[channel_name] = channel_handler
        ...
        try:
            ...
            # Use lock for reading shared state that might be updated
            with self.lock:
                existing_space = self.created_spaces.get(channel_name)
            
            if self.update_mode and existing_space:
                ...
                log_with_context(
                    logging.INFO,
                    f"[UPDATE MODE] Using existing space {space_id} for channel {channel_name}",
                    channel=channel_name,
                )
                with self.lock:
                    self.space_cache[channel_name] = space
            else:
                ...
                log_with_context(
                    logging.INFO,
                    f"{'[DRY RUN] ' if self.dry_run else ''}Step 1/6: {action_desc} for {channel_name}",
                    channel=channel_name,
                )
                
                # Check cache first with lock
                with self.lock:
                    space = self.space_cache.get(channel_name)
                
                if not space:
                    space = create_space(self, channel_name)
            ...
            if space and space.startswith("ERROR_NO_PERMISSION_"):
                log_with_context(
                    logging.WARNING,
                    f"Skipping channel {channel_name} due to space creation permission error",
                    channel=channel_name,
                )
                return

            # Set current space for file attachments (thread-local)
            self.current_space = space
            
            with self.lock:
                self.channel_to_space[channel_name] = space
                # Store in created_spaces for future reference
                self.created_spaces[channel_name] = space

            # Log that we're setting the current space
            log_with_context(
                logging.DEBUG,
                f"Setting current space to {space} for channel {channel_name} and storing in channel_to_space mapping",
                channel=channel_name,
            )
            ...
            if not self.update_mode:
                ...
                log_with_context(
                    logging.INFO,
                    f"{'[DRY RUN] ' if self.dry_run else ''}Step 2/6: Adding historical memberships for {channel_name}",
                    channel=channel_name,
                )
                add_users_to_space(self, space, channel_name)
            else:
                log_with_context(
                    logging.INFO,
            ...
            log_with_context(
                logging.INFO,
                f"{mode_prefix if self.dry_run or self.update_mode else ''} Step 3/6: Processing messages for {channel_name}",
                channel=channel_name,
            )
            ...
            # Use list(glob) to avoid iterator issues if FS changes (unlikely)
            json_files = sorted(list(ch_dir.glob("*.json")))
            ...
            if duplicate_count > 0:
                log_with_context(
                    logging.INFO,
                    f"Deduplicated {duplicate_count} messages in channel {channel_name} (likely thread reply duplicates)",
                    channel=channel_name,
                )
            ...
            if self.dry_run:
                ...
                log_with_context(
                    logging.INFO,
                    f"{mode_prefix} Found {message_count} messages in channel {channel_name}",
                    channel=channel_name,
                )
            ...
            if not self.dry_run or self.update_mode:
                # Thread-safe discovery needed? _discover_channel_resources uses self.channel_to_space (read only mostly)
                # and self.last_processed_timestamps (write). needs lock?
                # It writes to self.last_processed_timestamps[channel]. 
                # Since channel is unique per thread, this write is safe if dict is thread-safe (Python dicts are atomic for single item assignment)
                # But let's be safe.
                self._discover_channel_resources(channel_name)
            ...
            progress_desc = f"{mode_prefix}Adding messages to {channel_name}"
            # tqdm is thread-safe for display (uses lock internally), but multiple bars might be messy
            # But we are using it for iteration.
            pbar = tqdm(msgs, desc=progress_desc, leave=False)
                ...
                if result:
                    ...
                    if processed_count > 0:  # Avoid division by zero
                        ...
                        if failure_percentage > max_failure_percentage:
                            log_with_context(
                                logging.WARNING,
                                f"Failure rate {failure_percentage:.1f}% exceeds threshold {max_failure_percentage}% for channel {channel_name}",
                                channel=channel_name,
                            )
                            ...
                            with self.lock:
                                ...
                                self.high_failure_rate_channels[channel_name] = failure_percentage
            ...
            if channel_failures:
                with self.lock:
                    ...
                    self.failed_messages_by_channel[channel_name] = channel_failures
                channel_had_errors = True

            log_with_context(
                logging.INFO,
                f"Channel {channel_name} message import: processed {processed_count}, failed {failed_count}",
                channel=channel_name,
            )
            ...
            if not self.update_mode:
                log_with_context(
                    logging.INFO,
                    f"{'[DRY RUN] ' if self.dry_run else ''}Step 4/6: Completing import mode for {channel_name}",
                    channel=channel_name,
                )
                ...
                if (
                    ...
                    try:
                        ...
                        log_with_context(
                            logging.INFO,
                            f"{'[DRY RUN] ' if self.dry_run else ''}Step 5/6: Adding current members to space for {channel_name}",
                            channel=channel_name,
                        )

                        try:
                            add_regular_members(self, space, channel_name)
                            log_with_context(
                                logging.DEBUG,
                                f"Successfully added current members to space {space} for channel {channel_name}",
                                channel=channel_name,
                            )
                        except Exception as e:
                            log_with_context(
                        ...
                        with self.lock:
                            ...
                            self.incomplete_import_spaces.append((space, channel_name))
            ...
            log_with_context(
                logging.DEBUG,
                f"Channel log file completed for channel: {channel_name}",
                channel=channel_name,
            )
            
            # Check abort condition
            if self._should_abort_import(channel_name, processed_count, failed_count):
                if hasattr(self, "stop_execution"):
                    self.stop_execution.set()
            ...
            if channel_had_errors and not self.dry_run and not self.update_mode:
                self._delete_space_if_errors(space_name, channel_name)

        except Exception as e:
            log_with_context(
                logging.ERROR,
                f"Validation/Migration failed for channel {channel_name}: {e}",
                channel=channel_name,
                exc_info=True,
            )
            # Add to errors
            with self.lock:
                self.migration_errors.append(f"{channel_name}: {str(e)}")
                self.channels_with_errors.append(channel_name)
        finally:
            # We specifically DO NOT close the handler here because user might want to inspect it
            # and generic cleanup will handle it.
            # But to save file descriptors, maybe we should? 
            # The original code cleans up everything at the end.
            # In parallel, we might run out of FDs if we open 400 files.
            # So, better to close/remove handler here.
            with self.lock:
                handler = self.channel_handlers.pop(channel_name, None)
            
            if handler:
                try:
                    handler.flush()
                    handler.close()
                    logging.getLogger("slack_migrator").removeHandler(handler)
                except Exception:
                    pass
